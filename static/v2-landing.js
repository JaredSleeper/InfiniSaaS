...
(function () {
  ...
  function pageRow(r, showProject) {
    ...
    return `<tr data-lp-row="${p.id}">
      <td>
        <div><strong>${href ? `<a href="${esc(href)}" target="_blank" rel="noopener">${esc(p.name)}</a>` : esc(p.name)}</strong> ${badge(p.status)}${p.source === "pagedrones" ? ` <span class="badge" title="Published PageDrones alert template — synced from /alerts; subscribers = users who created a drone from it">⚡ alert · ${num((p.meta || {}).subscribers || 0)} subs</span>` : ""}</div>
  ...
  function backlogRow(p, st, showProject, projectsById) {
    ...
    return `<tr class="${st.selected.has(p.id) ? "selected" : ""}">
      ...
      <td>
        <div><strong>${esc(p.name)}</strong> ${p.status !== "idea" ? badge(p.status) : ""}${p.source === "agent" ? `<span class="muted" style="font-size:11px" title="Generated by the landing page agent"> ✦</span>` : ""}${((p.meta || {}).pagedrones_batches || []).length ? ` <span class="badge" title="Alert batches generated in PageDrones from this theme">⚡ ${p.meta.pagedrones_batches.length}</span>` : ""}</div>
